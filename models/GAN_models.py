...
class GAN(object):
    ...
    def _read_input_queue(self, filename_queue):
        ...
        print("Shuffling")
        input_image = tf.train.batch([read_input.input_image], batch_size=self.batch_size,
    ...
    def _gan_loss(self, logits_real, logits_fake, logits_random, feature_real, feature_fake, use_features=False):
    ...
    def create_network(self, generator_dims, discriminator_dims, optimizer="Adam", learning_rate=2e-4,
        ...
        epsilon = tf.random_uniform(shape=tf.shape(self.images), minval=0., maxval=1.)
        self.random_images = self.images + epsilon * (self.images - self.gen_images)
        discriminator_random_prob, logits_random, feature_random = self._discriminator(self.random_images,
                                                                                       discriminator_dims,
                                                                                       self.train_phase,
                                                                                       activation=leaky_relu,
                                                                                       scope_name="discriminator",
        ...
        # Loss calculation
        self._gan_loss(logits_real, logits_fake, logits_random, feature_real, feature_fake,
    ...
    def initialize_network(self, logs_dir, xla):
    ...
    def train_model(self, max_iterations):
        try:
            ...
            for itr in tqdm(range(1, max_iterations)):
                ...
                if itr % 10 == 0:
                    ...
                    tqdm.write('generator loss: {}, discriminator_loss: {}'.format(g_loss_val, d_loss_val))
...
class WasserstienGAN(GAN):
    def __init__(self, z_dim, crop_image_size, resized_image_size, batch_size, data_dir, clip_values=(-0.01, 0.01),
                 critic_iterations=5, lambda_val=10):
    ...
    def _discriminator(self, input_images, dims, train_phase, activation=tf.nn.relu, scope_name="discriminator",
        ...
        return None, h_pred, None  # Return the last convolution output. None values are returned to maintatin disc
        # from other GAN
    ...
    def _gan_loss(self, logits_real, logits_fake, logits_random, feature_real, feature_fake, use_features=False):
        ...
        grad_logits_random = tf.gradients(logits_random, [self.random_images])[0]
        slopes = tf.sqrt(tf.reduce_sum(tf.square(grad_logits_random)))
        gradient_penalty = tf.reduce_mean((slopes - 1.) ** 2)
        self.discriminator_loss += self.lambda_val * gradient_penalty
